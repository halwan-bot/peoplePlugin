    ...
    .controller('ContentPeopleCtrl', ['$scope', 'Location', '$modal', 'Buildfire', 'TAG_NAMES', 'STATUS_CODE', '$routeParams', 'RankOfLastItem', '$rootScope',
      function ($scope, Location, $modal, Buildfire, TAG_NAMES, STATUS_CODE, $routeParams, RankOfLastItem, $rootScope) {
        ...
        ContentPeople.getItem = function (itemId) {
          Buildfire.publicData.getById(itemId, TAG_NAMES.PEOPLE, function (err, item) {
            if (err)
        ...
        ContentPeople.addNewItem = function () {
          ...
          Buildfire.publicData.insert(ContentPeople.item.data, TAG_NAMES.PEOPLE, false, function (err, data) {
        ...
        ContentPeople.updateItemData = function () {
          Buildfire.publicData.update(ContentPeople.item.id, ContentPeople.item.data, TAG_NAMES.PEOPLE, function (err) {
            ContentPeople.isUpdating = false;
            if (err)
              return console.error('There was a problem saving your data');
          })
        ...
        ContentPeople.onUpdateFn = Buildfire.publicData.onUpdate(function (event) {
            ...
            switch (event.status) {
              case STATUS_CODE.INSERTED:
                ...
                Buildfire.publicData.get(TAG_NAMES.PEOPLE_INFO, function (err, result) {
                  ...
                  Buildfire.publicData.save(result.data, TAG_NAMES.PEOPLE_INFO, function (err) {
                    if (err)
